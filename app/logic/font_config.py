"""
字体配置模板
包含fonts.conf和42-repl-global.conf的模板内容
"""
...
class FontConfigManager:
    ...
    @staticmethod
    def generate_fonts_conf(font_name, font_filename):
        """生成fonts.conf内容"""
        content = FONTS_CONF_TEMPLATE.replace("FONTNAME", font_name)
        content = content.replace("FONTFILENAME", font_filename)
        return content
    
    @staticmethod
    def generate_global_conf(font_name):
        """生成42-repl-global.conf内容"""
        return GLOBAL_CONF_TEMPLATE.replace("FONTNAME", font_name)