...
class SoundReplacer:
    ...
    def replace_sound(self, sound_file_path):
        ...
        try:
            ...
            # 创建 customsounds 文件夹
            custom_sounds_dir = os.path.join(self.steam_path, "game", "csgo", "sounds", "customsounds")
            ...
            # Source 2 (CS2) 需要带 sounds 前缀，且通常建议带上 .vsnd_c 后缀
            cfg_content = f"sleep 3\nstopsound\nplay sounds/customsounds/{file_name_with_ext}"
            ...
            lines = [line for line in lines if "exec_async op" not in line.strip() and "exec op" not in line.strip()]
    ...
    def restore_sound(self):
        ...
        try:
            ...
            # 从 autoexec.cfg 中移除 exec_async op 和 exec op 命令
            autoexec_path = os.path.join(cfg_dir, "autoexec.cfg")
            if os.path.exists(autoexec_path):
                with open(autoexec_path, "r", encoding='utf-8') as f:
                    lines = f.readlines()
                
                # 移除包含 exec_async op 或 exec op 的行
                ...
                if removed_count > 0:
                    self._log(f"已从 autoexec.cfg 中移除 {removed_count} 行执行命令")
                else:
                    self._log("autoexec.cfg 中未找到执行命令")
            else:
